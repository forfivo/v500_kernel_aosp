...
export MODEL=v500;
export TYPE=stock;
export ANYKERNEL_DIR=~/Documentos/anykernel/;
export CROSS_COMPILE=~/Documentos/sabermod-4.9/bin/arm-eabi-
export ARCH=arm
...
echo "Removing old modules files from anykernel folder";
...
echo "Copying zImage to anykernel folder";
cp arch/arm/boot/zImage "$ANYKERNEL_DIR/kernel"

echo "Copying modules to anykernel folder";
for i in `find $KERNEL_DIR -name '*.ko'`; do
