...
static int anx7808_avdd_onoff_evbd(bool on)
{
	static bool power_state = 0;
	static struct regulator *anx7808_avdd_reg = NULL;
	int rc = 0;

	if (power_state == on) {
		pr_info("anx7808 avdd is already %s \n", power_state ? "on" : "off");
		goto out;
	}

	if (!anx7808_avdd_reg) {
		anx7808_avdd_reg= regulator_get(NULL, "slimport_avdd");
		if (IS_ERR(anx7808_avdd_reg)) {
			rc = PTR_ERR(anx7808_avdd_reg);
			pr_err("%s: regulator_get anx7808_avdd_reg failed. rc=%d\n",
					__func__, rc);
			anx7808_avdd_reg = NULL;
			goto out;
		}
		rc = regulator_set_voltage(anx7808_avdd_reg, 3300000, 3300000);
		if (rc ) {
			pr_err("%s: regulator_set_voltage anx7808_avdd_reg failed\
				rc=%d\n", __func__, rc);
			goto out;
		}
	}
	...
	if (on) {
		rc = regulator_set_optimum_mode(anx7808_avdd_reg, 100000); //FIXME - need to check of load current
		if (rc < 0) {
			pr_err("%s : set optimum mode 100000, anx7808_avdd_reg failed \
					(%d)\n", __func__, rc);
			goto out;
		}
		rc = regulator_enable(anx7808_avdd_reg);
		if (rc) {
			pr_err("%s : anx7808_avdd_reg enable failed (%d)\n",
					__func__, rc);
			goto out;
		}
	}
	else {
		rc = regulator_disable(anx7808_avdd_reg);
		if (rc) {
			pr_err("%s : anx7808_avdd_reg disable failed (%d)\n",
				__func__, rc);
			goto out;
		}
		rc = regulator_set_optimum_mode(anx7808_avdd_reg, 100);
		if (rc < 0) {
			pr_err("%s : set optimum mode 100, anx7808_avdd_reg failed \
				(%d)\n", __func__, rc);
			goto out;
		}
	}
...
{
	/* LGE_CHANGE_START:2013-05-28:ilhwan.ahn@lge.com:Runtime configuration for slimport */
	if (lge_get_board_revno() == HW_REV_EVB2)
		anx7808_pdata.avdd_power = anx7808_avdd_onoff_evbd;
	else /* Disable ANX_AVDD33_EN for current save. */
		pm8xxx_gpio_config(ANX_AVDD33_EN, &pm_gpio_disable);
	/* LGE_CHANGE_END:2013-05-28:ilhwan.ahn@lge.com:Runtime configuration for slimport */
...
#define GPIO_RC_EN    PM8921_GPIO_PM_TO_SYS(18)
#else
...
static unsigned hall_ic_gpio[] = {
	GPIO_CFG(GPIO_POUCH_INT, 0, GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
};
