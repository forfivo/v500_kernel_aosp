...
//LGE_CHANGE_S, moon-wifi@lge.com by wo0ngs 2013-06-17, wifi dma qos control
#if defined (CONFIG_LGE_BCM433X_PATCH) && defined (CONFIG_BCM4334)
	{
...
#endif
//LGE_CHANGE_E, moon-wifi@lge.com by wo0ngs 2013-06-17, wifi dma qos control
}
	...
	for (i = 0; i < data->sg_len; i++) {
		...
		while (len > 0) {
			if (len > SPS_MAX_DESC_SIZE) {
				...
				(sps_pipe_handle ==host->sps.cons.pipe_handle)) {
					/*
					set EOT only for consumer pipe, for producer pipe h/w will set it.
					*/
